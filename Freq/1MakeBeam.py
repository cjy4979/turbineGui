...
DataBeam=np.loadtxt(StrFile,skiprows=2,encoding='utf-8-sig')
#Abaqus中的梁单元只能在xy平面内构建，轴线方向为x方向。
#后面在assembly中通过旋转，将轴线改为z向
#所以此处的节点坐标放在了ax中
ax=DataBeam[:,0]#节点坐标。
ar=DataBeam[:,1]#横截面外径R1
at=DataBeam[:,2]#横截面壁厚t=R1-R0
...
##创建梁与实体的Tie连接
#mdb.models['Model-1'].Tie(name='BeamSolidTie', master=region1, slave=region2, 
